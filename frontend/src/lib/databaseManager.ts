...
class ClientDatabaseManager {
  ...
  getActiveConnection(): DatabaseConnection | null {
    try {
      ...
      const cacheData: DatabaseConnectionCache = JSON.parse(cached)
      if (!cacheData.activeConnection) return null

      const connections = cacheData.connections || []
      return connections.find(conn => conn.url === cacheData.activeConnection) || null
    } catch (error) {
  ...
  parseConnectionUrl(url: string): Partial<DatabaseConnection> {
    try {
      ...
      return {
        url,
        type: type === 'postgres' || type === 'postgresql' ? 'postgresql' : type as DatabaseConnection['type'],
  ...
  generateConnectionName(connection: DatabaseConnection): string {
    ...
    return connection.url.substring(0, 30) + (connection.url.length > 30 ? '...' : '')
  }
  ...
  getDatabasePreferences(): {
    ...
    try {
      ...
      return prefs ? JSON.parse(prefs) : { autoConnect: true, maxConnections: 5, connectionTimeout: 30000 }
    } catch (error) {
      console.warn('Failed to get database preferences:', error)
      return { autoConnect: true, maxConnections: 5, connectionTimeout: 30000 }
    }
  }
  ...
  validateDatabaseUrl(url: string): { valid: boolean; error?: string } {
    try {
      const urlObj = new URL(url)
      const supportedProtocols = ['postgresql:', 'postgres:', 'mysql:', 'sqlite:']
      ...
      return {
        valid: false,
        error: 'Invalid URL format. Example: postgresql://user:password@host:5432/database'
      }
    }
  }
  ...
  getConnectionStats() {
    ...
    return {
      ...
      activeConnectionName: activeConnection ? this.generateConnectionName(activeConnection) : null,
