...
export class EnclaveClient {
   ...
   */
  addConnection(instanceId: string, enclaveId: string, host: string, port: number = 8080): void {
  ...
  async generateWalletKeys(
    ...
    try {
      console.log(`🔑 Generating wallet keys in enclave for instance: ${instanceId}`);
  ...
  async signTransaction(
    ...
    try {
      console.log(`✍️ Signing transaction in enclave for instance: ${instanceId}`);
  ...
  async reconstructKey(
    instanceId: string,
    shares: Array<{ index: number; encryptedShare: string }>
  ): Promise<{ walletAddress: string; publicKey: string; keyReconstructed: boolean }> {
    const connection = this.getConnection(instanceId);
    if (!connection) {
      throw new Error(`No enclave connection found for instance: ${instanceId}`);
    }
    ...
    try {
      console.log(`🔓 Reconstructing key in enclave for instance: ${instanceId}`);
  ...
  async getAttestation(instanceId: string): Promise<any> {
    ...
    try {
      console.log(`📋 Getting attestation from enclave for instance: ${instanceId}`);
  ...
  private async makeRequest(
    ...
    try {
      ...
      if (error.name === 'TypeError' && error.message.includes('fetch')) {
        throw new Error('Failed to connect to enclave - check if enclave is running');
      }
      throw error;
    }
  }
   ...
   */
  async mockGenerateWalletKeys(walletName: string, userId: string): Promise<WalletGenerationResult> {
   ...
   */
  async mockSignTransaction(request: TransactionSigningRequest): Promise<SignedTransactionResult> {
